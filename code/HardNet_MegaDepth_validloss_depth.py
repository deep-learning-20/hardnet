...
class HardNet(nn.Module):
    ...
    def input_norm(self,x):
        ...
        meanindex=flat[:,3,:].nonzero()
        mp=torch.mean(flat[:,3,:][(meanindex[:, 0], meanindex[:, 1])])
        sp=torch.std(flat[:,3,:][(meanindex[:, 0], meanindex[:, 1])])
...
"""
    ...
    import pickle
    writefile = open("/cluster/scratch/tianhu/test/train123.pkl", "wb")
    pickle.dump(data, writefile)
    writefile.close()
    """
