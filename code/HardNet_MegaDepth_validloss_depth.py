...
from lib.dataset_depthconcat import MegaDepthDataset
import sys
...
class HardNet(nn.Module):
    ...
    def input_norm(self,x):
        ...
        mp = torch.mean(flat[:,:3,:], dim=2)
        sp = torch.std(flat[:,:3,:], dim=2) + 1e-7


        x[:,:3,:,:]=(x[:,:3,:,:] - mp.detach().unsqueeze(-1).unsqueeze(-1).expand_as(x[:,:3,:,:])) / sp.detach().unsqueeze(-1).unsqueeze(-1).expand_as(x[:,:3,:,:])

        #mp=torch.mean(flat[:,3,:][flat[:,3,:].nonzero(as_tuple=True)].view(-1, 1))
        #sp=torch.std(flat[:,3,:][flat[:,3,:].nonzero(as_tuple=True)].view(-1, 1)) + 1e-7



        x[:,3,:,:]=x[:,3,:,:]/torch.max(x[:,3,:,:])
        return x
    def forward(self, input):
...
if __name__ == '__main__':
    ...
    scene_list_path='/cluster/scratch/tianhu/hardnet/code/megadepth_utils/train_scenes.txt',
    preprocessing='caffe',
    image_size=32
)
    ...
    validation_dataset = MegaDepthDataset(
        scene_list_path='/cluster/scratch/tianhu/hardnet/code/megadepth_utils/valid_scenes.txt',
        train=False,
        preprocessing='caffe',
#        pairs_per_scene=25,
        image_size=32
