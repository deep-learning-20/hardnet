...
class MegaDepthDataset(Dataset):
    def __init__(
            ...
            scene_info_path='/cluster/scratch/rthakur/dl/scene_info',
            base_path='/cluster/scratch/rthakur/dl',
            train=True,
            preprocessing=None,
            min_overlap_ratio=.5,
            max_overlap_ratio=1,
    ...
    def build_dataset(self):
        ...
        #print("WTF",self.scenes)
        #print("WTF is happening?")

        for scene in tqdm(self.scenes[:1], total=len(self.scenes[:1])):
            scene_info_path = os.path.join(
                self.scene_info_path, '%s.0.npz' % scene
            )
            #print("here is the path",scene_info_path)
    ...
    def __getitem__(self, idx):
        ...
        depth1=np.reshape(depth1,(-1,self.image_size,self.image_size))
        depth2=np.reshape(depth2,(-1,self.image_size,self.image_size))
        #print(image1.shape)
        #print(depth1.shape)
        image1=np.concatenate([image1,depth1],axis=0)
        image2=np.concatenate([image2,depth2],axis=0)
...
"""
        return {
            ...
            'intrinsics1': intrinsics1.astype(np.float32),
            'pose1': pose1.astype(np.float32),
            'bbox1': bbox1.astype(np.float32),
            'image2': image2.astype(np.float32),
            'depth2': depth2.astype(np.float32),
            'intrinsics2': intrinsics2.astype(np.float32),
            'pose2': pose2.astype(np.float32),
            'bbox2': bbox2.astype(np.float32)
        }
"""