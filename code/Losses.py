...
def loss_transform(theta_a,theta_p):
    theta=torch.cat((theta_a,theta_p),0)
    theta_translation=theta[:,:,2]
    theta_mat=theta[:,:,:2]
    loss1=torch.norm(theta_translation)
    loss2=torch.mean(torch.abs(theta_mat[:,0,0]*theta_mat[:,1,1]-theta_mat[:,0,1]*theta_mat[:,1,0]))
